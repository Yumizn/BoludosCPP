// Includes &/or Headers

#include <iostream>
#include <ostream>
#include "menu.hpp"
#include "argentina.hpp"



// Namespaces/Workspaces

using namespace std;



void argentina()
{
	std::cout << "Hola Manco de mierda" << std::endl;
	std::cout << "1 real = 999999999999 pesos!" << std::endl;
	std::cout << "Brasil tiene 5 copas del mundo y argentina tiene 3!" << std::endl;
	std::cout << "Solo pele tiene 3 mundiales y messi no!" << std::endl;
	std::cout << "Solo julga a dead center y dan rage en los otros mapas!" << std::endl;
	std::cout << "El juego consiste en sumar puntos, ¡quien haga mas puntos gana!" << std::endl;
	std::cout << "En 15 anos de Left 4 Dead aun no han aprendido a jugar" << std::endl;
	std::cout << "no obtienes puntos por matar infectados! solo por llegar mas lejos" << std::endl;
	std::cout << "rushear es la forma correcta de ganar!" << std::endl;
	std::cout << "Cuanto antes llegues a la casa segura,mayor será la garantía de ganar el juego" << std::endl;
	std::cout << "argentino vive en un pais en bancarrota donde 99999999 pesos no valen 1 real" << std::endl;
	std::cout << "A los tramposos no los expulsas y por eso siempre pierdes y cuando ganas no hay gloria ni honor" << std::endl;
}

